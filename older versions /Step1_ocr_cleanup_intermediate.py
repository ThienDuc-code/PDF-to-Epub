...
def remove_headers_footers(text: str, log: dict) -> str:
    ...
    def has_vol(line): return re.search(r"(?i)\bvolume\s+(seven|7)\b", line) is not None
    def has_ch(line): return re.search(r"(?i)\bchapter\b", line) is not None
    ...
    single_pat = re.compile(r"^\s*(?:[•â€¢·]\s*)?(?i:volume\s+seven)\s+(?i:chapter)\s+[A-Za-z]+\s*$")
    extra = 0; cleaned2 = []
    for ln in cleaned:
    ...
    for idx, ln in enumerate(cleaned2):
        ...
        if re.match(r"^\s*(?i:volume\s+seven)\s*$", ln):
            ...
            if re.match(r"^\s*([\.,•â€¢·\-\u2010\u2011\u2013\u2014\'\"”’])?\s*(?i:chapter)\s+[A-Za-z]+\s*$", nxt):
                two_count += 1; skip_next = True; continue
        cleaned3.append(ln)

    # Inline "Volume Seven Chapter X"
    inline_pat = re.compile(r"(?i:volume\s+seven)\s+(?i:chapter)\s+[A-Za-z]+")
