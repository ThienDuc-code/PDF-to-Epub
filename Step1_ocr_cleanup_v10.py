...
def remove_header_footer_blocks(text: str, log: dict) -> str:
    ...
    for i in range(n):
        ...
        if vol_re.search(s) or hist_re.search(s):
            lo = i
            while lo - 1 >= 0 and is_headerish(lo - 1) and not allcaps_chapter_heading_re.match(lines[lo-1].strip()):
                lo -= 1
            hi = i
            while hi + 1 < n and is_headerish(hi + 1) and not allcaps_chapter_heading_re.match(lines[hi+1].strip()):
                hi += 1
            ...
            def absorb_up(lo0):
                ...
                while k >= 0 and taken < 3:
                    s = lines[k].strip()
                    if s and (numeric_only_re.match(s) or punct_only_re.match(s) or quote_only_re.match(s)):
